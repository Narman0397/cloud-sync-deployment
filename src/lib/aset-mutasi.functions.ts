...
export const resolveAsetByToken = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { data: row, error } = await supabaseAdmin
      .from("aset")
      .select(
        "id,kode,qr_token,nama,kategori,merk,nomor_seri,opd_id,pemegang_user_id,lokasi_terkini,lat,lng,status,foto_url, opd:opd!opd_id(nama,singkatan), pemegang:profiles!pemegang_user_id(nama_lengkap)",
      )
...
export const ajukanMutasi = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (
      !c.isSuper &&
      !(c.isAdminOpd && c.opdId === a.opd_id) &&
      context.userId !== a.pemegang_user_id
    ) {
...
export const listMutasi = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = supabaseAdmin
      .from("aset_mutasi")
      .select(
        "id,aset_id,dari_user,ke_user,dari_opd,ke_opd,alasan,status,approved_by,approved_at,catatan_approval,created_at, aset:aset!aset_id(kode,nama), opd_dari:opd!dari_opd(singkatan), opd_ke:opd!ke_opd(singkatan), pemegang_dari:profiles!dari_user(nama_lengkap), pemegang_ke:profiles!ke_user(nama_lengkap)",
      )
...
const pemSchema = z.object({
  ...
  selesai_at: z.string().datetime().optional().nullable(),
...
export const upsertPemeliharaan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload = { ...data, opd_id: a.opd_id, created_by: context.userId };
    if (data.id) {
...
export const listPemeliharaan = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) =>
    z.object({ aset_id: z.string().uuid().optional().nullable() }).parse(i),
  )
  .handler(async ({ data }) => {
    let q = supabaseAdmin
      .from("aset_pemeliharaan")
      .select(
        "id,aset_id,jadwal,jenis,deskripsi,status,selesai_at,biaya,vendor,created_at, aset:aset!aset_id(kode,nama)",
      )
...
export const listNilaiBuku = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const c = await ctxOf(context.userId);
    let q = supabaseAdmin
      .from("aset_nilai_buku")
      .select(
        "id,kode,nama,opd_id,nilai_perolehan,tanggal_perolehan,umur_ekonomis_bulan,metode_susut,nilai_buku",
      )
      .order("nilai_buku", { ascending: false })
      .limit(500);
    if (!c.isSuper) {
...
export const generateQrLabelPdf = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = supabaseAdmin
      .from("aset")
      .select("id,kode,qr_token,nama,opd_id")
      .in("id", data.aset_ids);
    ...
    return {
      ...
      filename: `qr-label-${Date.now()}.pdf`,
      count: rows.length,
    };
  });
...
export const listDueWarranty = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const c = await ctxOf(context.userId);
    const { data: rows, error } = await supabaseAdmin.rpc("aset_due_warranty", {
      _days: data.days,
    });
    if (error) throw new Error(error.message);
