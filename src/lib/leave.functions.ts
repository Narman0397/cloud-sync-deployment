...
async function assertLeaveAdmin(userId: string, targetUserId: string) {
  ...
  if (ctx.isAdminOpd) {
    const { data: tgt } = await supabaseAdmin
      ...
      .eq("id", targetUserId)
      .maybeSingle();
    if ((tgt?.opd_id as string | null) === ctx.opdId) return ctx;
  }
...
export const listLeaveBalances = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Pegawai non-admin dipaksa hanya melihat saldo miliknya.
    const canSeeOthers = ctx.isSuper || ctx.isElevated || ctx.isAdminOpd;
    if (!canSeeOthers && data.user_id && data.user_id !== context.userId) {
      throw new Error("Forbidden");
    }
    let q = supabaseAdmin
      .from("leave_balances")
      .select(
        "id,user_id,tahun,jenis,kuota,terpakai,catatan,updated_at, profile:profiles!user_id(nama_lengkap,opd_id)",
      )
      .order("updated_at", { ascending: false })
      .limit(500);
    if (data.tahun) q = q.eq("tahun", data.tahun);
    if (data.user_id) q = q.eq("user_id", data.user_id);
    else if (!canSeeOthers) q = q.eq("user_id", context.userId);
    const { data: rows, error } = await q;
    if (error) throw new Error(error.message);
    // Admin OPD dibatasi ke pegawai di OPD-nya
    let filtered = (rows ?? []) as unknown as Array<{
      profile?: { opd_id?: string | null } | null;
    }>;
    if (!(ctx.isSuper || ctx.isElevated) && ctx.isAdminOpd && ctx.opdId) {
      filtered = filtered.filter((r) => (r.profile?.opd_id ?? null) === ctx.opdId);
    }
...
export const upsertLeaveBalance = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertLeaveAdmin(context.userId, data.user_id);
    ...
    await supabaseAdmin.from("audit_log").insert({
      ...
      aksi: "leave.balance_upsert",
      entitas: "leave_balances",
      data_sesudah: {
        user_id: data.user_id,
        tahun: data.tahun,
        jenis: data.jenis,
...
export const approveLeaveWithBalance = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Otorisasi: hanya super/elevated atau admin_opd pada OPD pegawai target.
    await assertLeaveAdmin(context.userId, izin.user_id);

    const start = new Date(izin.dari);
    const end = new Date(izin.sampai);
    ...
    if (izin.mengurangi_saldo) {
      const { data: bal } = await supabaseAdmin
        .from("leave_balances")
        .select("id,kuota,terpakai")
        .eq("user_id", izin.user_id)
        .eq("tahun", tahun)
        .eq("jenis", izin.jenis)
        .maybeSingle();
      const sisa = (bal?.kuota ?? 0) - (bal?.terpakai ?? 0);
      if (sisa < hari)
        throw new Error(`Saldo ${izin.jenis} ${tahun} kurang (sisa ${sisa}, butuh ${hari})`);
    ...
    await supabaseAdmin.from("audit_log").insert({
      user_id: context.userId,
      aksi: "leave.approve",
      entitas: "pengajuan_izin",
      entitas_id: data.izin_id,
      data_sesudah: { hari, jenis: izin.jenis },
    });
    return { ok: true, hari };
  });
