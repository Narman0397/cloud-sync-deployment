...
const layananSchema = z.object({
  ...
  // Standar Pelayanan (UU 25/2009 & PermenPAN-RB 15/2014)
  dasar_hukum: z.string().max(2000).optional().nullable(),
  biaya: z.string().max(500).optional().nullable(),
  produk_layanan: z.string().max(500).optional().nullable(),
  jam_pelayanan: z.string().max(300).optional().nullable(),
  sarana_prasarana: z.string().max(1000).optional().nullable(),
  kompetensi_pelaksana: z.string().max(1000).optional().nullable(),
  jumlah_pelaksana: z.number().int().min(0).max(9999).optional().nullable(),
  jaminan_pelayanan: z.string().max(1000).optional().nullable(),
  jaminan_keamanan: z.string().max(1000).optional().nullable(),
  mekanisme_pengaduan: z.string().max(1000).optional().nullable(),
  evaluasi_kinerja: z.string().max(1000).optional().nullable(),
  maklumat_pelayanan: z.string().max(2000).optional().nullable(),
  faq: z
    .array(z.object({ q: z.string().min(1).max(300), a: z.string().min(1).max(2000) }))
    .max(50)
    .default([]),
  document_template_id: z.string().uuid().optional().nullable(),
  tte_required: z.boolean().default(false),
  tte_signer_role: z.enum(["kepala_opd", "kabid", "staf"]).optional().nullable(),
});
...
export const upsertLayanan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload = {
      ...
      produk_layanan: data.produk_layanan ?? null,
      jam_pelayanan: data.jam_pelayanan ?? null,
      sarana_prasarana: data.sarana_prasarana ?? null,
      kompetensi_pelaksana: data.kompetensi_pelaksana ?? null,
      jumlah_pelaksana: data.jumlah_pelaksana ?? null,
      jaminan_pelayanan: data.jaminan_pelayanan ?? null,
      jaminan_keamanan: data.jaminan_keamanan ?? null,
      mekanisme_pengaduan: data.mekanisme_pengaduan ?? null,
