...
export const listPayrollPeriods = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const isElevated = ctx.isSuper || ctx.isElevated;
    if (!isElevated && !ctx.isAdminOpd) throw new Error("Forbidden");
    // Admin OPD dibatasi ke opd sendiri; filter opd_id yang berbeda ditolak.
    if (!isElevated && ctx.isAdminOpd) {
      if (data.opd_id !== undefined && data.opd_id !== ctx.opdId) throw new Error("Forbidden");
    }
    let q = supabaseAdmin
      .from("payroll_periods")
      .select(
        "id,opd_id,tahun,bulan,locked_at,locked_by,unlocked_at,unlocked_by,catatan,created_at, opd:opd!opd_id(nama,singkatan)",
      )
      .order("tahun", { ascending: false })
      .order("bulan", { ascending: false })
      .limit(120);
    if (!isElevated) q = q.eq("opd_id", ctx.opdId as string);
    else if (data.opd_id !== undefined)
      q = data.opd_id === null ? q.is("opd_id", null) : q.eq("opd_id", data.opd_id);
...
export const lockPayrollPeriod = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: superRes } = await supabaseAdmin.rpc("has_role", {
      _user_id: context.userId,
      _role: "super_admin",
    });
    if (!superRes) {
      const { data: opdRes } = await supabaseAdmin.rpc("has_role", {
        _user_id: context.userId,
        _role: "admin_opd",
      });
...
export const unlockPayrollPeriod = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) =>
    z.object({ id: z.string().uuid(), catatan: z.string().max(500).optional() }).parse(i),
  )
  .handler(async ({ data, context }) => {
    const { data: superRes } = await supabaseAdmin.rpc("has_role", {
      _user_id: context.userId,
