...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    let q = supabase
      .from("permohonan")
      .select(
        "id,kode,judul,kategori,status,tanggal_masuk,tenggat,updated_at,opd_id,pemohon_id,petugas_id",
      )
    ...
    <AdminShell opdAktifId={opdAktifId} onChangeOpd={isSuperAdmin ? setOpdAktifId : undefined}>
      <div className="mb-6">
        <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
          {opd?.singkatan ?? (isAdminDesa && profile?.desa ? `Desa ${profile.desa}` : "Semua OPD")}
        </div>
        <h1 className="font-display text-2xl font-bold text-foreground md:text-3xl">
          {isSuperAdmin ? "Command Center Super Admin" : "Dashboard Permohonan Warga"}
        </h1>
        <p className="text-sm text-muted-foreground">
          {isSuperAdmin
            ? "Pantau kesehatan sistem, antrian kerja, dan 5 ekosistem portal dalam satu layar."
            : "Pantau & proses pengajuan layanan publik secara terpusat."}
        </p>
      </div>

      {isSuperAdmin && <SuperCommandCenter summary={summary} />}
            ...
            <div className="rounded-xl border border-border bg-card p-4 shadow-soft">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="rounded-lg bg-surface p-3">
                  ...
                  <div
                    className={`font-display text-xl font-bold ${sysStat && sysStat.jobs.failed > 0 ? "text-destructive" : ""}`}
                  >
                    {sysStat?.jobs.failed ?? "—"}
                  </div>
                </div>
          ...
          <div className="mt-4 rounded-xl border border-border bg-card p-4 shadow-soft">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  % selesai dalam batas SLA per kategori
                </p>
              </div>
            ...
            {slaPerformance.length === 0 ? (
              ...
              <ul className="mt-3 grid gap-2 md:grid-cols-2">
                {slaPerformance.map((s) => {
                  const tone =
                    s.persen >= 80 ? "bg-success" : s.persen >= 50 ? "bg-gold" : "bg-destructive";
                  return (
                    <li key={s.nama} className="rounded-lg border border-border bg-surface p-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-semibold">{s.nama}</span>
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs font-bold ${s.persen >= 80 ? "bg-success/15 text-success" : s.persen >= 50 ? "bg-gold/20 text-gold-foreground" : "bg-destructive/15 text-destructive"}`}
                        >
                          {s.persen}%
                        </span>
                      </div>
                      ...
                      <div className="mt-1 text-[10px] text-muted-foreground">
                        {s.total} selesai
                        {s.targetDays != null ? ` · target ${s.targetDays} hari` : ""}
                      </div>
                    </li>
                  );
                })}
            ...
            <div className="mt-4 flex gap-3 rounded-xl border border-destructive/40 bg-destructive/10 p-4 text-sm">
              ...
              <div className="flex-1">
                <div className="font-semibold text-foreground">
                  Ada {sysStat.jobs.failed} job gagal
                </div>
                <p className="text-xs text-muted-foreground">
                  Periksa dari halaman audit / backup.
                </p>
              </div>
              <Link
                to="/admin/audit"
                className="self-center rounded-md bg-destructive px-3 py-1.5 text-xs font-semibold text-destructive-foreground"
              >
                Lihat
              </Link>
            </div>
      ...
      {!(isAdminDesa && !isSuperAdmin) && (
        <div className="mt-6 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border bg-card p-4 shadow-soft">
          <div>
            <div className="font-display text-base font-semibold">Daftar Permohonan</div>
            <p className="text-xs text-muted-foreground">
              Filter, cari, dan kelola permohonan di halaman tersendiri.
            </p>
          </div>
          <Link
            to="/admin/permohonan"
          ...
          >
            Buka Halaman Permohonan <ArrowUpRight className="h-3 w-3" />
          </Link>
        </div>
      )}
    </AdminShell>
