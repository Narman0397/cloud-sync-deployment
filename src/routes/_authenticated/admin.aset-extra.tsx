// Admin: Mutasi Aset, Pemeliharaan, dan QR Label PDF.
...
import { ArrowLeftRight, Wrench, QrCode, Check, X, Plus, Save } from "lucide-react";

export const Route = createFileRoute("/_authenticated/admin/aset-extra")({
  head: () => ({
    meta: [{ title: "Admin — Aset Lanjutan" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <AdminShell>
...
type Tab = "mutasi" | "pemeliharaan" | "qr_label";
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Mutasi aset, jadwal pemeliharaan, dan cetak label QR aset.
        </p>
      </div>
...
function TabBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center gap-1 px-3 py-2 text-sm font-medium ${active ? "border-b-2 border-primary text-primary" : "text-muted-foreground"}`}
    >
      {icon} {children}
    </button>
  );
...
function MutasiTab() {
  ...
  const load = useCallback(async () => {
    setBusy(true);
    try {
      const r = (await listMutasi({ data: {} })) as unknown as { rows: MutasiRow[] };
      setRows(r.rows);
    } finally {
      setBusy(false);
    }
  ...
  async function decide(id: string, status: "approved" | "rejected") {
    const catatan =
      status === "rejected" ? (prompt("Alasan penolakan (min 5 karakter):") ?? "") : "";
    if (status === "rejected" && catatan.length < 5) return;
    try {
      await putusanMutasi({ data: { id, status, catatan } });
      await load();
    } catch (e) {
            ...
            {rows.map((r) => (
              <tr key={r.id}>
                <td className="px-3 py-2">
                  <div className="font-medium">{r.aset?.nama}</div>
                  <div className="font-mono text-xs text-muted-foreground">{r.aset?.kode}</div>
                </td>
                <td className="px-3 py-2 text-xs">
                  {r.opd_dari?.singkatan ?? "—"} → {r.opd_ke?.singkatan ?? "—"}
                  <div className="text-[10px] text-muted-foreground">
                    {r.pemegang_dari?.nama_lengkap ?? "-"} → {r.pemegang_ke?.nama_lengkap ?? "-"}
                  </div>
                </td>
                <td className="px-3 py-2 text-xs">{r.alasan}</td>
                <td className="px-3 py-2">
                  <span
                    className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${r.status === "approved" ? "bg-success/15 text-success" : r.status === "rejected" ? "bg-destructive/15 text-destructive" : "bg-amber-500/15 text-amber-700"}`}
                  >
                    {r.status}
                  </span>
                </td>
                  ...
                  {r.status === "pending" && (
                    <div className="flex gap-1">
                      <button
                        ...
                        className="rounded-md border border-success/40 bg-success/10 p-1.5 text-success"
                        title="Setujui"
                      >
...
function MutasiForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    try {
      await ajukanMutasi({
        data: { aset_id: asetId, ke_user: keUser || null, ke_opd: keOpd || null, alasan },
      });
      onSaved();
    } catch (e) {
      alert(e instanceof Error ? e.message : "Gagal");
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg rounded-xl bg-card p-6 shadow-elegant"
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input
              value={keUser}
              onChange={(e) => setKeUser(e.target.value)}
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
              placeholder="UUID pemegang baru"
            />
          </div>
          <div>
            ...
            <input
              value={keOpd}
              onChange={(e) => setKeOpd(e.target.value)}
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
              placeholder="UUID OPD penerima"
            />
          </div>
          <div>
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <button
            onClick={save}
            disabled={busy || !asetId || alasan.length < 5}
...
function PemeliharaanTab() {
  ...
  const [form, setForm] = useState<{
    ...
    biaya: string;
    vendor: string;
  }>({ aset_id: "", jadwal: "", jenis: "", biaya: "", vendor: "" });

  const load = useCallback(async () => {
    setBusy(true);
    try {
      const [r, a] = await Promise.all([listPemeliharaan({ data: {} }), listAset({ data: {} })]);
  ...
  async function save() {
    if (!form.aset_id || !form.jadwal || !form.jenis) return alert("Lengkapi aset, jadwal, jenis");
    try {
      await upsertPemeliharaan({
        data: {
      ...
      setForm({ aset_id: "", jadwal: "", jenis: "", biaya: "", vendor: "" });
      await load();
    } catch (e) {
      alert(e instanceof Error ? e.message : "Gagal");
    }
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <div className="grid gap-2 sm:grid-cols-5">
          <select
            value={form.aset_id}
            onChange={(e) => setForm({ ...form, aset_id: e.target.value })}
        ...
        <button
          onClick={save}
          className="mt-3 inline-flex items-center gap-1 rounded-md bg-gradient-primary px-3 py-2 text-sm font-semibold text-primary-foreground shadow-soft"
        >
          ...
          <tbody className="divide-y divide-border">
            ...
            {rows.map((r) => (
              <tr key={r.id}>
                <td className="px-3 py-2 text-xs">
                  <div className="font-medium">{r.aset?.nama}</div>
                  <div className="font-mono text-[10px] text-muted-foreground">{r.aset?.kode}</div>
                </td>
                <td className="px-3 py-2 text-xs">{r.jadwal}</td>
...
function QrLabelTab() {
  ...
  async function generate() {
    ...
    try {
      ...
      const r = (await generateQrLabelPdf({
        data: { aset_ids: Array.from(selected), base_url: base },
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {aset.map((a) => (
          <label
            key={a.id}
            className="flex cursor-pointer items-center gap-2 rounded-md border border-border bg-card p-3 text-sm hover:border-primary"
          >
