...
export const Route = createFileRoute("/_authenticated/admin/users")({
  head: () => ({
    meta: [{ title: "Manajemen User — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <UsersPage />
...
type Row = {
  ...
  pendingRole?: AppRoleUI;
  pendingOpd?: string | null;
  pendingDesa?: string | null;
};
...
function UsersPage() {
  ...
  async function saveRole(row: Row) {
    ...
    try {
      ...
      const opd_id =
        role === "admin_opd" || role === "asn" ? (row.pendingOpd ?? row.opd_id ?? null) : null;
      const desa =
        role === "admin_desa" ? (row.pendingDesa ?? row.desa ?? "").trim() || null : null;
  ...
  async function toggleSuspend(row: Row) {
    ...
    setActId(row.id);
    try {
      await setUserSuspended({ data: { user_id: row.id, suspend } });
  ...
  async function logout(row: Row) {
    ...
    setActId(row.id);
    try {
      await forceSignOut({ data: { user_id: row.id } });
  ...
  async function reset(row: Row) {
    ...
    setActId(row.id);
    try {
      await sendPasswordReset({ data: { email: row.email } });
  ...
  async function toggleVerify(row: Row) {
    ...
    if (
      !confirm(
        verify
          ? `Verifikasi akun ${row.email} sebagai ${row.role}?`
          : `Cabut verifikasi akun ${row.email}?`,
      )
    )
      return;
    setActId(row.id);
    ...
    return rows
      .filter(
        (r) =>
          r.role === "warga" &&
          matchQ(r) &&
          (wargaDesa === "semua" || (r.desa ?? "") === wargaDesa),
      )
      .sort(
        (a, b) =>
          (a.desa ?? "zzz").localeCompare(b.desa ?? "zzz") ||
        ...
        <tr className="border-t border-border align-top">
          ...
          <td className="px-4 py-3">
            <select
              ...
              onChange={(e) =>
                setRows((prev) =>
                  prev.map((p) =>
                    p.id === r.id ? { ...p, pendingRole: e.target.value as AppRoleUI } : p,
                  ),
                )
              }
            ...
            >
              ...
              <option value="admin_pemda">Admin Pemda</option>
              <option value="pimpinan">Pimpinan Daerah</option>
              <option value="admin_bkpsdm">Admin BKPSDM</option>
              <option value="kepala_bkpsdm">Kepala BKPSDM</option>
          ...
          <td className="px-4 py-3">
            {role === "admin_desa" ? (
              <select
                ...
                onChange={(e) =>
                  setRows((prev) =>
                    prev.map((p) => (p.id === r.id ? { ...p, pendingDesa: e.target.value } : p)),
                  )
                }
              ...
              <select
                ...
                onChange={(e) =>
                  setRows((prev) =>
                    prev.map((p) =>
                      p.id === r.id ? { ...p, pendingOpd: e.target.value || null } : p,
                    ),
                  )
                }
                className="h-9 rounded-md border border-border bg-background px-2 text-sm disabled:opacity-50"
            ...
            {dirty && (
              <button
                onClick={() => saveRole(r)}
                disabled={
                  busy || (needsOpd && !opd) || (role === "admin_desa" && desa.trim().length < 2)
                }
                className="ml-2 inline-flex items-center gap-1 rounded-md bg-primary px-2.5 py-1.5 text-xs font-semibold text-primary-foreground disabled:opacity-40"
              >
          ...
          <td className="px-4 py-3">
            <div className="flex flex-col gap-1">
              <span
                className={`inline-flex w-fit rounded-full px-2 py-0.5 text-xs ${suspended ? "bg-destructive/15 text-destructive" : "bg-success/15 text-success"}`}
              >
                {suspended ? "Suspended" : "Aktif"}
              </span>
              {(canVerify || r.role === "super_admin") && (
                <span
                  className={`inline-flex w-fit items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold ${r.verified_at ? "bg-primary/15 text-primary" : "bg-destructive/15 text-destructive"}`}
                >
                  {r.verified_at ? (
                    <>
          ...
          <td className="px-4 py-3 text-xs">
            {r.asn_type || r.system_position ? (
              <div className="space-y-0.5">
                ...
                {r.system_position && (
                  <div className="text-muted-foreground">{POSITION_LABEL[r.system_position]}</div>
                )}
              </div>
            ) : (
              <button
          ...
          <td className="px-4 py-3">
            <div className="flex flex-wrap justify-end gap-1.5">
              <button
                ...
                className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs ${suspended ? "border-success/40 text-success hover:bg-success/10" : "border-destructive/40 text-destructive hover:bg-destructive/10"} disabled:opacity-40`}
              >
                {suspended ? (
                  <>
              ...
              {canVerify && (
                <button
                  ...
                  disabled={busy}
                  className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs ${r.verified_at ? "border-amber-500/40 text-amber-700 dark:text-amber-400 hover:bg-amber-500/10" : "border-primary/40 text-primary hover:bg-primary/10"} disabled:opacity-40`}
                >
                  {r.verified_at ? (
                    <>
                      <ShieldOff className="h-3 w-3" /> Cabut Verif
                    </>
                  ) : (
                    <>
                      <ShieldCheck className="h-3 w-3" /> Verifikasi
                    </>
                  )}
                </button>
              )}
    ...
    <AdminShell breadcrumb={[{ label: "Manajemen User" }]}>
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Pusat kontrol akun, peran, OPD, klasifikasi ASN, permission override, dan audit. Klik
            baris untuk membuka detail RBAC.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <select
            value={fAsnType}
            onChange={(e) => setFAsnType(e.target.value)}
            className="h-9 rounded-md border border-border bg-background px-2 text-xs"
            aria-label="Filter tipe ASN"
          >
            <option value="semua">Semua Tipe ASN</option>
            {Object.values(ASN_TYPES).map((v) => (
              <option key={v} value={v}>
                {ASN_TYPE_LABEL[v as AsnType]}
              </option>
            ))}
          </select>
          <select
      ...
      {loadError && (
        <div className="mb-4 rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm">
          ...
          <button
            onClick={() => load()}
            className="mt-3 inline-flex h-8 items-center rounded-md bg-destructive px-3 text-xs font-semibold text-destructive-foreground"
          >
            Coba lagi
          </button>
        </div>
