...
export const Route = createFileRoute("/_authenticated/pemda")({
  head: () => ({
    meta: [{ title: "Dashboard Admin Pemda" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <header>
          ...
          <p className="text-sm text-muted-foreground">
            Monitoring seluruh OPD — SLA, layanan, audit, absensi, aset, dan pelaporan data.
          </p>
        </header>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {MONITOR_LINKS.map((m) => (
              <Link
                key={m.to}
                to={m.to}
                className="flex items-center gap-3 rounded-xl border border-border bg-card p-4 shadow-soft hover:bg-primary-soft hover:text-primary"
              >
...
function Stat({
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 shadow-soft">
      ...
      <div
        className={`mt-2 font-display text-2xl font-bold ${tone === "destructive" ? "text-destructive" : ""}`}
      >
