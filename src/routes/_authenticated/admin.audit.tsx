...
function AuditPage() {
  ...
  const fetchPage = useCallback(
    async (f: Filters, cur?: string) => {
      const res = (await listFn({ data: buildPayload(f, cur) } as never)) as {
        items: Row[];
    ...
    <AdminShell breadcrumb={[{ label: "Audit Log" }]}>
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Filter dan ekspor jejak audit. Dimuat: {rows.length}.
          </p>
        </div>
        <button
          onClick={onExport}
      ...
      <div className="mb-4 grid gap-2 rounded-xl border border-border bg-card p-3 shadow-soft sm:grid-cols-2 lg:grid-cols-4">
        ...
        <input
          ...
          onChange={(e) => setFilters({ ...filters, entitas: e.target.value })}
          placeholder="Entitas (mis. permohonan)"
          className="rounded-md border border-border bg-background px-3 py-2 text-sm"
        />
        <input
          value={filters.aksi}
          onChange={(e) => setFilters({ ...filters, aksi: e.target.value })}
          placeholder="Aksi (mis. update)"
          className="rounded-md border border-border bg-background px-3 py-2 text-sm"
        />
        <input
          value={filters.entitas_id}
          onChange={(e) => setFilters({ ...filters, entitas_id: e.target.value })}
