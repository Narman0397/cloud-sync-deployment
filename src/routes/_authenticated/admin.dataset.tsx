...
function Page() {
  ...
  const load = useCallback(async () => {
    setBusy(true);
    try {
      const r = (await listTemplatesAdmin()) as unknown as { rows: Tpl[] };
      setRows(r.rows);
    } finally {
      setBusy(false);
    }
  ...
  async function onExport(id: string) {
    try {
      const r = (await exportSubmissionsXlsx({ data: { template_id: id } })) as unknown as {
        url: string;
        filename: string;
      };
      ...
      alert(ex instanceof Error ? ex.message : "Gagal ekspor");
    }
  }
  ...
  async function onMigrate(id: string, judul: string) {
    if (
      !confirm(
        `Pindahkan template "${judul}" ke Form Builder?\n\nTemplate akan dikloning menjadi form baru (kolom, target, & status). Template lama akan dinonaktifkan.`,
      )
    )
      return;
    try {
      const r = (await migrateDatasetToForm({ data: { template_id: id } })) as unknown as {
        form_id: string;
      };
      alert("Berhasil dipindahkan. Form ID: " + r.form_id);
      await load();
    } catch (ex) {
      alert(ex instanceof Error ? ex.message : "Gagal migrasi");
    }
  }
  ...
  return (
    <div>
      <div className="mb-4 rounded-xl border border-amber-300/50 bg-amber-50 px-4 py-3 text-xs text-amber-900 dark:bg-amber-950/30 dark:text-amber-200">
        <strong>Modul lama.</strong> Pengisian dataset baru menggunakan{" "}
        <a href="/admin/form-builder" className="font-semibold underline">
          Form Builder
        </a>{" "}
        dengan workflow assignment, review, dan audit log lengkap.
      </div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="font-display text-xl font-bold">Manajemen Dataset</h2>
          <p className="text-sm text-muted-foreground">
            Buat template formulir untuk diisi ASN, lalu ekspor rangkuman ke Excel.
          </p>
        </div>
        <a
          href="/admin/form-builder/wizard"
        ...
        >
          <Plus className="h-4 w-4" /> Buat di Form Builder
        </a>
      </div>
            ...
            {rows.map((t) => (
              <tr key={t.id}>
                ...
                <td className="px-3 py-2">
                  <span
                    className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${t.aktif ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}
                  >
                ...
                <td className="px-3 py-2">
                  <div className="flex items-center gap-1">
                    <IconBtn onClick={() => setViewSubs(t.id)} title="Lihat Submission">
                      <Eye className="h-3.5 w-3.5" />
                    </IconBtn>
                    <IconBtn onClick={() => onExport(t.id)} title="Ekspor Excel">
                      <FileSpreadsheet className="h-3.5 w-3.5" />
                    </IconBtn>
                    <IconBtn onClick={() => onToggle(t.id, t.aktif)} title="Toggle Aktif">
                      <Power className="h-3.5 w-3.5" />
                    </IconBtn>
                    <IconBtn
                      onClick={() => onMigrate(t.id, t.judul)}
                      title="Pindahkan ke Form Builder"
                    >
      ...
      {viewSubs && (
        <SubmissionsModal
          template_id={viewSubs}
          onClose={() => setViewSubs(null)}
          onExport={() => onExport(viewSubs)}
        />
      )}
    </div>
  );
...
function TemplateForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const [targetScope, setTargetScope] = useState<"opd_sendiri" | "lintas_opd" | "spesifik">(
    "opd_sendiri",
  );
  const [deadline, setDeadline] = useState("");
  const [allowMulti, setAllowMulti] = useState(false);
  const [kolom, setKolom] = useState<Kolom[]>([
    { key: "kegiatan", label: "Nama Kegiatan", tipe: "text", required: true },
  ]);
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState<string | null>(null);

  function addKolom() {
    setKolom((p) => [...p, { key: `kol_${p.length + 1}`, label: "Kolom Baru", tipe: "text" }]);
  }
  function updKolom(i: number, patch: Partial<Kolom>) {
    setKolom((p) => p.map((k, j) => (j === i ? { ...k, ...patch } : k)));
  }
  function delKolom(i: number) {
    setKolom((p) => p.filter((_, j) => j !== i));
  }
    ...
    try {
      await upsertTemplate({
        data: {
          ...
          aktif: true,
          allow_multiple_submit: allowMulti,
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Field label="Ruang Lingkup">
              <select
                className="input"
                value={targetScope}
                onChange={(e) => setTargetScope(e.target.value as typeof targetScope)}
              >
                <option value="opd_sendiri">OPD Sendiri</option>
                <option value="lintas_opd">Lintas OPD</option>
          ...
          <div className="rounded-lg border border-border p-3">
            ...
            <div className="space-y-2">
              {kolom.map((k, i) => (
                <div
                  key={i}
                  className="grid grid-cols-12 gap-2 rounded border border-border p-2 text-xs"
                >
                  <input
                    ...
                    value={k.key}
                    onChange={(e) =>
                      updKolom(i, { key: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, "_") })
                    }
                  />
                  <input
                    className="input col-span-3"
                    placeholder="Label"
                    value={k.label}
                    onChange={(e) => updKolom(i, { label: e.target.value })}
                  />
                  <select
                    className="input col-span-2"
                    value={k.tipe}
                    onChange={(e) => updKolom(i, { tipe: e.target.value as Kolom["tipe"] })}
                  >
                  ...
                  <label className="col-span-1 inline-flex items-center justify-center">
                    <input
                      type="checkbox"
                      checked={!!k.required}
                      onChange={(e) => updKolom(i, { required: e.target.checked })}
                      title="Wajib"
                    />
                  </label>
                  <button
                    onClick={() => delKolom(i)}
                    className="col-span-12 inline-flex items-center justify-end text-destructive"
                  >
                    <X className="h-3 w-3" />
                  </button>
                </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              onClick={onSave}
              disabled={busy}
              className="inline-flex items-center gap-1 rounded-md bg-gradient-primary px-3 py-2 text-sm font-semibold text-primary-foreground shadow-soft disabled:opacity-60"
            >
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-foreground/40 p-4">
      <div className="w-full max-w-5xl rounded-xl bg-background shadow-elegant">
        ...
        <div className="max-h-[70vh] overflow-auto p-5">
          ...
          {!busy && rows.length === 0 && (
            <div className="text-sm text-muted-foreground">Belum ada submission.</div>
          )}
          {!busy && rows.length > 0 && (
