...
export const Route = createFileRoute("/_authenticated/admin/security/permissions")({
  head: () => ({
    meta: [{ title: "Permission Management — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <SuperAdminOnly>
  ...
  return (
    <AdminShell breadcrumb={[{ label: "Security" }, { label: "Permissions" }]}>
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Kelola permission user. Semua perubahan tercatat di audit log.
      </p>
      <div className="mb-4 inline-flex rounded-md border border-border bg-card p-1 text-sm">
        <button
          onClick={() => setTab("catalog")}
          className={`rounded px-3 py-1 ${tab === "catalog" ? "bg-primary text-primary-foreground" : ""}`}
        >
          Catalog
        </button>
        <button
          onClick={() => setTab("users")}
          className={`rounded px-3 py-1 ${tab === "users" ? "bg-primary text-primary-foreground" : ""}`}
        >
          User → Permission
        </button>
      </div>
