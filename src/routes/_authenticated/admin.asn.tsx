  ...
  return (
    <AdminShell breadcrumb={[{ label: "Modul ASN" }]}>
      ...
      <p className="text-sm text-muted-foreground">
        Pengaturan QR kantor (titik koordinat + radius) dan monitoring absensi.
      </p>
      <div className="mt-4 inline-flex flex-wrap rounded-lg border border-border bg-surface p-1">
        {(["kantor", "absensi"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`h-9 px-4 rounded-md text-sm font-semibold ${tab === t ? "bg-gradient-primary text-primary-foreground" : "text-muted-foreground"}`}
          >
...
function KantorTab({ opds }: { opds: Opd[] }) {
  ...
  const [edit, setEdit] = useState<
    Record<string, { lat: string; lng: string; radius: string; lokasi: string; label: string }>
  >({});

  async function reload() {
    const r = await listKantorQR();
    const rs = (r as unknown as { rows: Qr[] }).rows;
    setRows(rs);
    setEdit((prev) => {
      const next = { ...prev };
      for (const q of rs) {
  ...
  function ensureEdit(opdId: string) {
    setEdit((prev) =>
      prev[opdId]
        ? prev
        : { ...prev, [opdId]: { lat: "", lng: "", radius: "100", lokasi: "", label: "" } },
    );
  }
  ...
  function pickGps(opdId: string) {
    ...
    navigator.geolocation.getCurrentPosition(
      (p) => {
        setField(opdId, "lat", p.coords.latitude.toFixed(6));
        setField(opdId, "lng", p.coords.longitude.toFixed(6));
        toast.success("Koordinat GPS diisi");
      },
  ...
  async function save(opdId: string, rotate = false) {
    ...
    if (Number.isNaN(lat) || Number.isNaN(lng)) {
      toast.error("Koordinat lat/lng wajib diisi");
      return;
    }
    const radius = Math.max(10, Math.min(5000, Number(e?.radius || 100)));
    setBusyId(opdId);
    try {
      await regenerateKantorQR({
        data: {
          opd_id: opdId,
          lat,
          lng,
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-md border border-border bg-surface p-3 text-xs text-muted-foreground">
        Tetapkan <b>titik koordinat</b> dan <b>radius</b> setiap kantor OPD. ASN hanya bisa absen
        jika berada dalam radius tersebut (default 100&nbsp;m). Gunakan tombol{" "}
        <i>Pakai GPS sekarang</i> bila Anda sedang berada di lokasi kantor.
      </div>
      ...
      {opds.map((o) => {
        ...
        const url = q
          ? `${typeof window !== "undefined" ? window.location.origin : ""}/asn/scan/${q.token}`
          : "";
        return (
            ...
            <div className="flex flex-wrap items-start gap-4">
              <div className="flex-1 min-w-[240px]">
                <div className="font-semibold">
                  {o.nama} <span className="text-xs text-muted-foreground">({o.singkatan})</span>
                </div>
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(ev) => {
                        ensureEdit(o.id);
                        setField(o.id, "lat", ev.target.value);
                      }}
                      placeholder="-2.5489"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(ev) => {
                        ensureEdit(o.id);
                        setField(o.id, "lng", ev.target.value);
                      }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(ev) => {
                        ensureEdit(o.id);
                        setField(o.id, "radius", ev.target.value);
                      }}
                      className="input h-9 w-full"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(ev) => {
                        ensureEdit(o.id);
                        setField(o.id, "label", ev.target.value);
                      }}
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(ev) => {
                        ensureEdit(o.id);
                        setField(o.id, "lokasi", ev.target.value);
                      }}
                      placeholder="Jl. ..."
                      className="input h-9 w-full"
                    />
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    onClick={() => pickGps(o.id)}
                    className="inline-flex h-9 items-center gap-1 rounded-md border border-border px-3 text-xs"
                  >
                    <MapPin className="h-3 w-3" /> Pakai GPS sekarang
                  </button>
                  <button
                    onClick={() => save(o.id, false)}
                    disabled={busyId === o.id}
                    className="inline-flex h-9 items-center gap-1 rounded-md bg-gradient-primary px-3 text-xs font-semibold text-primary-foreground disabled:opacity-60"
                  >
                    {busyId === o.id ? (
                    ...
                    {q ? "Simpan" : "Generate QR"}
                  </button>
                  {q && (
                    <button
                      onClick={() => save(o.id, true)}
                      disabled={busyId === o.id}
                      className="inline-flex h-9 items-center gap-1 rounded-md border border-border px-3 text-xs"
                    >
                  ...
                  {q && (
                    <button
                      onClick={() => downloadQrPng(url, `qr-${o.singkatan}.png`)}
                      className="inline-flex h-9 items-center gap-1 rounded-md border border-border px-3 text-xs"
                    >
                  ...
                  <div className="mt-2 text-[11px] text-muted-foreground break-all">
                    ...
                    {q.lat !== null && q.lng !== null && (
                      <>
                        ...
                        · Koordinat aktif:{" "}
                        <b>
                          {Number(q.lat).toFixed(5)}, {Number(q.lng).toFixed(5)}
                        </b>{" "}
                        · Radius: <b>{q.radius_m ?? 100} m</b>
                      </>
                    )}
                  </div>
                )}
              ...
              <div className="flex flex-col items-center gap-2">
                {q ? (
                  <QrImage value={url} size={130} />
                ) : (
                  <div className="grid h-32 w-32 place-items-center rounded-md border border-dashed border-border text-xs text-muted-foreground text-center px-2">
                    Belum ada QR
                  </div>
                )}
              </div>
...
type AbsRow = {
  ...
  lng: number | null;
  opd: { nama: string; singkatan: string } | null;
  profile: { nama_lengkap: string; nip: string | null; jabatan: string | null } | null;
};
function AbsensiTab({ opds }: { opds: Opd[] }) {
  ...
  async function reload() {
    const r = await listAbsensiAdmin({
      data: {
        opd_id: opdId || null,
    ...
    setRows((r as unknown as { rows: AbsRow[] }).rows);
  }
  useEffect(() => {
    reload().catch(() => {});
  }, []);
  ...
  function exportCsv() {
    ...
    const body = filtered
      .map(
        (r) =>
          `"${r.profile?.nama_lengkap ?? ""}","${r.profile?.nip ?? ""}","${r.profile?.jabatan ?? ""}","${r.opd?.singkatan ?? ""}",${r.tipe},${new Date(r.waktu).toISOString()},${r.lat ?? ""},${r.lng ?? ""}`,
      )
      .join("\n");
    const blob = new Blob([header + body], { type: "text/csv" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `absensi-${Date.now()}.csv`;
    a.click();
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <input
          ...
          placeholder="Cari nama / NIP / OPD"
          className="h-9 rounded-md border border-border bg-background px-2 text-sm flex-1 min-w-[180px]"
        />
        <button
          onClick={reload}
        ...
        <button onClick={exportCsv} className="h-9 rounded-md border border-border px-3 text-xs">
          Export CSV
        </button>
      </div>
              ...
              >
                <td className="px-3 py-2">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {r.profile?.nip ?? ""} {r.profile?.jabatan ? `· ${r.profile.jabatan}` : ""}
                  </div>
                </td>
                <td className="px-3 py-2">{r.opd?.singkatan ?? "-"}</td>
                <td className="px-3 py-2">
                  <span
                    className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${r.tipe === "masuk" ? "bg-success/15 text-success" : "bg-accent/20 text-accent"}`}
                  >
                    {r.tipe.toUpperCase()}
                  </span>
                </td>
