  ...
  head: () => ({
    meta: [
      { title: "Kontak & LAPOR! — Pemerintah Kabupaten Buton Selatan" },
      {
        name: "description",
        content:
          "Hubungi Pemerintah Kabupaten Buton Selatan atau sampaikan laporan & aspirasi melalui kanal LAPOR!.",
      },
...
function KontakPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const ticket = (data as { ticket_code: string | null } | null)?.ticket_code ?? null;
      setLastTicket(ticket);
      toast.success(
        ticket ? `Laporan terkirim! Nomor tiket: ${ticket}` : "Laporan terkirim!",
      );
      setForm({
        nama: "",
        nik: "",
    ...
    <PageShell>
      ...
      {lastTicket && (
        <section className="container-page pt-8">
          <div className="mx-auto max-w-2xl rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-5">
            <p className="text-sm font-semibold text-emerald-800 dark:text-emerald-300">
              Laporan Anda tercatat.
            </p>
            <p className="mt-2 text-sm text-muted-foreground">
              Simpan nomor tiket berikut untuk memantau tindak lanjut:
            </p>
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <code className="rounded-md border border-emerald-500/40 bg-white px-3 py-1.5 font-mono text-sm font-bold text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200">
                {lastTicket}
              </code>
              <Link
                to="/lapor/$ticket"
                params={{ ticket: lastTicket }}
          ...
          <div className="mx-auto max-w-2xl rounded-xl border border-amber-500/30 bg-amber-500/5 p-5 text-sm">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <Link
                  to="/auth"
                  search={{ redirect: "/kontak" } as never}
                  className="mt-3 inline-flex h-9 items-center rounded-md bg-gradient-primary px-4 text-xs font-semibold text-primary-foreground"
                >
                  Masuk Akun
                </Link>
              </div>
          ...
          >
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              <Field label="Lokasi Kejadian" className="md:col-span-2">
                <input
                  ...
                  placeholder="Kelurahan, kecamatan, atau alamat"
                />
              </Field>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                Data Anda dilindungi sesuai UU Perlindungan Data Pribadi.
              </p>
              <button
                disabled={busy}
                className="inline-flex h-12 items-center gap-2 rounded-md bg-gradient-primary px-7 text-sm font-semibold text-primary-foreground shadow-soft disabled:opacity-60"
              >
