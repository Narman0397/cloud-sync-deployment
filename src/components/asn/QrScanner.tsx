...
export function QrScanner({ onResult, paused }: Props) {
  ...
  // Deteksi iframe (preview Lovable) — kamera sering diblok oleh kebijakan
  // izin iframe. Tawarkan "Buka di tab baru" untuk keluar dari iframe.
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-black/90">
      ...
      {(inIframe || !isSecure) && phase === "idle" && !error && (
        <div className="space-y-2 bg-amber-500/15 px-3 py-2 text-xs text-amber-900 dark:text-amber-200">
          <div>
            {inIframe
              ? "Kamera tidak bisa berjalan di dalam preview iframe."
              : "Halaman ini bukan HTTPS — kamera tidak dapat diakses."}
          </div>
          {inIframe && (
            <button
              type="button"
