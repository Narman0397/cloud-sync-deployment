...
export function PermissionGate({
  ...
  const roleOk = role
    ? (Array.isArray(role) ? role : [role]).some((r) =>
        ...
        (roles as any[]).includes(r as any),
      )
    : true;
...
export function RoleBadge({ role, className = "" }: { role: AppRole; className?: string }) {
  const tone: Record<AppRole, string> = {
    ...
    admin_bkpsdm: "bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-500/30",
    kepala_bkpsdm: "bg-violet-500/10 text-violet-700 dark:text-violet-400 border-violet-500/30",
