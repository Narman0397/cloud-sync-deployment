...
export type AuthzContext = {
  ...
  isAsn: boolean;
  isPimpinan: boolean;
  /** Admin BKPSDM — cross-OPD access untuk modul kepegawaian/ASN. */
  isAdminBkpsdm: boolean;
  /** Kepala BKPSDM — eksekutif kepegawaian (view + approve). */
...
export async function getUserContext(supabase: SB, userId: string): Promise<AuthzContext> {
  ...
  const isAdminBkpsdm = roleSet.has("admin_bkpsdm");
  const isKepalaBkpsdm = roleSet.has("kepala_bkpsdm");
  ...
  return {
    ...
    pimpinanType,
    // BKPSDM diperlakukan setara admin_pemda untuk akses lintas-OPD
    // pada modul kepegawaian / ASN.
    isElevated: isSuper || isPemda || isAdminBkpsdm,
    // Kepala BKPSDM memperoleh dashboard eksekutif (read-only) sama
    // seperti Pimpinan / Admin Pemda.
    isExecutiveView: isSuper || isPemda || isPimpinanRole || isKepalaBkpsdm,
  };
}
